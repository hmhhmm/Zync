"""
Live GLM integration tests — requires real GLM_API_KEY in .env
run with pytest tests/integration/test_pipeline_live.py -v -s
Run separately:
    pytest tests/integration/test_pipeline_live.py -v

These tests are NOT part of the normal pytest tests/ run.
They cost real API tokens and take 30-60 seconds each.
...
pytestmark = pytest.mark.skipif(
    ...
    reason="GLM_API_KEY not set — skipping live GLM tests"
)
...
async def test_live_agent2_chemist_streaming():
    """
    LIVE: Agent 2 calls real GLM with streaming.
    Expected:
    - receives at least 1 reasoning chunk
    ...
    # Must have streamed some reasoning
    assert len(reasoning_chunks) > 0, "No reasoning chunks received — streaming may be broken"
